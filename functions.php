...
if (!function_exists('restaurant_menu_price_calc')) {
    ...
    function restaurant_menu_price_calc($get_added_menus = '', $foodbakery_restaurant_id = '', $fee = false, $vat = false, $only_vat = false, $converter = false, $order_id = '', $currency_symbol = false, $foodbakery_discount_price = 0) {
        global $foodbakery_plugin_options;
        ...
        if (!is_string($get_added_menus) && isset($get_added_menus[$foodbakery_restaurant_id]) && !empty($get_added_menus[$foodbakery_restaurant_id]) && is_array($get_added_menus[$foodbakery_restaurant_id])) {
            ...
            foreach ($get_added_menus[$foodbakery_restaurant_id] as $menu_ord_item) {
                ...
                if (isset($menu_ord_item['menu_id']) && isset($menu_ord_item['price'])) {
                    ...
                    $this_item_quantity = isset( $menu_ord_item['quantity'] )? $menu_ord_item['quantity'] : 1;
        ...
        return foodbakery_get_currency($menu_t_price-$foodbakery_discount_price, $currency_symbol, '', '', $converter);
    }
    ...
    for ($menu_loop = 0; $menu_loop < $total_menu_count; $menu_loop ++) {
        ...
        for ($menu_items_loop = 0; $menu_items_loop < $total_items; $menu_items_loop ++) {
            if (isset($restaurant_menu_list[$menu_items_loop]['restaurant_menu']) && $total_menu[$menu_loop] == $restaurant_menu_list[$menu_items_loop]['restaurant_menu']) {
                ...
                $menu_item_stock = isset($restaurant_menu_list[$menu_items_loop]['menu_item_stock']) ? $restaurant_menu_list[$menu_items_loop]['menu_item_stock'] : 1000;
                ...
                $menu_item_identity = isset($restaurant_menu_list[$menu_items_loop]['menu_item_identity']) ? $restaurant_menu_list[$menu_items_loop]['menu_item_identity'] : '';
               ...
               $image_src_array_p = explode('wp-content', $menu_item_icon_img_src_p);
                ...
                <li>
                    ...
                    <div class="price-holder">
                        ...
                        <?php
                        $menu_unique_id = rand(10000, 4999999);
                        if (isset($menu_item_extra[0]['title']) && is_array($menu_item_extra[0]['title']) && sizeof($menu_item_extra[0]['title']) > 0) {
                            $ajax_url = admin_url('admin-ajax.php');
                            ?>
                            <a href="javascript:void(0);" data-menu_rand_id="<?php echo absint($menu_item_identity); ?>" data-item_stock="<?php echo absint($menu_item_stock); ?>" class="menu_rand_id_<?php echo absint($menu_item_identity); ?> dev-adding-menu-btn-<?php echo absint($menu_items_loop) ?>" onclick="foodbakery_show_extra_menu_item('extras-<?php echo absint($menu_loop) ?>-<?php echo absint($menu_items_loop) ?>', '<?php echo absint($menu_items_loop) ?>', '<?php echo absint($menu_loop) ?>', '<?php echo $ajax_url; ?>', '<?php echo $foodbakery_restaurant_id; ?>');"><i class="icon-plus4 text-color"></i></a>
                            <?php
                        } else {
                            ?>
                            <a href="javascript:void(0)" data-menu_rand_id="<?php echo absint($menu_item_identity); ?>" data-item_stock="<?php echo absint($menu_item_stock); ?>" data-menu_item_identity="<?php echo absint($menu_item_identity) ?>" class="menu_rand_id_<?php echo absint($menu_item_identity); ?> restaurant-add-menu-btn restaurant-add-menu-btn-<?php echo absint($menu_items_loop) ?>" data-rid="<?php echo absint($foodbakery_restaurant_id) ?>" data-id="<?php echo absint($menu_items_loop) ?>" data-cid="<?php echo absint($menu_loop) ?>"><i class="icon-plus4 text-color"></i></a>
                            <?php
                        }
                        ?>
                        <span id="add-menu-loader-<?php echo absint($menu_items_loop) ?>"></span>
                    </div>
                    <?php do_action('foodbakery_after_menu_item_data', $restaurant_menu_list, $menu_items_loop);?>
                </li>
        ...
        if ($menu_it_html != '') {
            ?>
            <div class="element-title" id="menu-category-<?php echo absint($menu_loop) ?>">
                <h5 class="text-color"><?php echo esc_html($total_menu[$menu_loop]); ?></h5>
                                            ...
                                            <div class="extras-detail-main" id="menu_idd_<?php echo $menu_extra_counter; ?>">
                                                ...
                                                <div class="extras-detail-options">
                                                    ...
                                                    foreach ($menu_item_extra_titles as $key => $menu_item_extra_title) {
                                                        ...
                                                        $all_id = 'extra-' . absint($menu_extra_att_counter) . '-' . absint($menu_extra_counter) . '-' . absint($this_item_ids);
                                                        $checked = '';
                                                        ...
                                                        <div class="extras-detail-att <?php echo $all_id; ?>">
                                                            <input type="<?php echo $field_type; ?>" <?php echo $checked; ?> id="extra-<?php echo absint($menu_extra_att_counter) ?>-<?php echo absint($menu_extra_counter) ?>-<?php echo absint($menu_items_loop) ?>" data-ind="<?php echo absint($menu_extra_att_counter) ?>" data-menucat-id="<?php echo absint($menu_loop) ?>" data-menu-id="<?php echo absint($this_item_ids) ?>" name="extra-<?php echo absint($menu_extra_counter) ?>-<?php echo absint($this_item_ids) ?>">
...
if (!function_exists('foodbakery_show_extra_menu_item')) {
    ...
    function foodbakery_show_extra_menu_item() {
        ...
        if (isset($menu_item_extra[0]['title']) && is_array($menu_item_extra[0]['title']) && sizeof($menu_item_extra[0]['title']) > 0) {
            ?>
            <div class="modal fade menu-extras-modal add_extrass" id="<?php echo $pop_up_id; ?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                            <h2><a><?php esc_html_e('Extras', 'foodbakery') ?></a></h2>
                        </div>
                        <div class="modal-body">
                            <div class="menu-selection-container">
                                <?php
